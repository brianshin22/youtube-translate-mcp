...
try:
    ...
    logger.error("Failed to import MCP library. Make sure it's installed.")
    raise
...
async def get_transcript(url: str) -> str:
    ...
    try:
        ...
        logger.info(f"Attempting to retrieve transcript directly for: {url}")
        ...
        if video_status and video_status.get("status") == "processing":
            logger.info(f"Video is still processing, waiting for completion")
            ...
            while attempts < max_attempts:
                ...
                delay = await calculate_backoff_delay(attempts)
                logger.info(f"Waiting {delay:.1f}s before checking video status, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
                ...
                if not video_status:
                    logger.info("Failed to get video status, will retry")
                    continue
        ...
        while attempts < max_attempts:
            ...
            if attempts > 0:
                ...
                logger.info(f"Waiting {delay:.1f}s before retrying transcript retrieval, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
            ...
            if transcript_response and "transcript" in transcript_response.get("metadata", {}):
                # Return the full transcript as text
                logger.info("Successfully retrieved transcript")
                return transcript_response["metadata"]["transcript"]["text"]
        ...
        logger.error(f"Exception during transcript retrieval: {str(e)}")
...
async def get_translation(url: str, language: str) -> str:
    ...
    try:
        ...
        logger.info(f"Attempting to retrieve translation directly for: {url}, language: {language}")
        ...
        logger.info(f"Checking if translation exists for: {video_id}, language: {language}")
        ...
        if not translation_status or translation_status.get("data", {}).get("status") != "completed":
            logger.info(f"Requesting translation for video ID: {video_id}, language: {language}")
            ...
            if not translate_response:
                logger.error(f"Failed to request translation to {language}")
            ...
            while attempts < max_attempts:
                ...
                logger.info(f"Waiting {delay:.1f}s before checking translation status, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
                ...
                if not translation_status:
                    logger.info("Failed to check translation status, will retry")
                    continue
                ...
                if status == "processing":
                    logger.info("Translation is still being processed, continuing to wait")
                
            if attempts >= max_attempts:
                logger.error("Translation processing timed out")
                return "Translation processing timed out. Try again later."
        else:
            logger.info(f"Translation already exists")
        ...
        while attempts < max_attempts:
            ...
            if attempts > 0:
                delay = await calculate_backoff_delay(attempts - 1)
                logger.info(f"Waiting {delay:.1f}s before retrying translation retrieval, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
            
            # Get the translated transcript
            logger.info(f"Retrieving translated transcript for video ID: {video_id}, language: {language}")
            ...
            if translated_transcript and "data" in translated_transcript:
                ...
                logger.info("Formatting translation response")
                if "text" in translated_transcript["data"]:
                    ...
                    logger.error("Translation format not recognized")
        ...
        logger.error(f"Exception during translation: {str(e)}")
...
async def get_subtitles(url: str, language: str = "en", format: str = "srt") -> str:
    ...
    try:
        ...
        logger.info(f"Generating subtitles for: {url}, language: {language}, format: {format}")
        if format.lower() not in ["srt", "vtt"]:
            logger.error(f"Invalid subtitle format: {format}")
        ...
        if language.lower() != "en":
            ...
            if not translation_status or translation_status.get("data", {}).get("status") != "completed":
                ...
                logger.info(f"Translation needed for {language}, will request it first")
                ...
                while attempts < max_attempts:
                    ...
                    if status == "error":
                        logger.error(f"Error translating to {language}")
                ...
                logger.info(f"Translation to {language} already exists")
                
        # Check if subtitles already exist
        logger.info(f"Checking if subtitles exist for: {video_id}, language: {language}, format: {format}")
        subtitle_status = await make_yt_api_request(
            f"/api/videos/{video_id}/subtitles/status?language={language}&format={format}"
        )
        
        # If subtitles don't exist or are not completed, request them
        if not subtitle_status or subtitle_status.get("data", {}).get("status") != "completed":
            logger.info(f"Requesting subtitle generation for video ID: {video_id}")
            ...
            if not subtitle_request:
                ...
                if translation_needed:
                    logger.info("Translation was just completed, waiting before retrying subtitles request")
                ...
                if not subtitle_request:
                    logger.error(f"Failed to request {format} subtitles in {language}")
            ...
            while attempts < max_attempts:
                ...
                logger.info(f"Waiting {delay:.1f}s before checking subtitle status, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
                ...
                if not subtitle_status:
                    logger.info("Failed to check subtitle status, will retry")
                    continue
                
                status = subtitle_status.get("data", {}).get("status")
                logger.info(f"Subtitle status: {status}")
                
                if status == "completed":
                    logger.info(f"Subtitle generation completed")
                    break
                    
                if status == "error":
                    error_msg = subtitle_status.get("data", {}).get("message", "Unknown error")
                    logger.error(f"Error generating subtitles: {error_msg}")
                    ...
                    if "translation" in error_msg.lower() and not translation_needed:
                        logger.info("Error suggests translation is needed, will request translation and retry")
                ...
                if status == "processing":
                    logger.info(f"Subtitle generation still in progress")
                
            if attempts >= max_attempts:
                logger.error("Subtitle generation timed out")
                return "Subtitle generation timed out. Try again later."
        else:
            logger.info(f"Subtitles already exist")
        ...
        while attempts < max_attempts:
            ...
            if attempts > 0:
                delay = await calculate_backoff_delay(attempts - 1)
                logger.info(f"Waiting {delay:.1f}s before retrying subtitles retrieval, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
            
            # Get the subtitles
            logger.info(f"Retrieving subtitles for video ID: {video_id}")
        ...
        logger.error(f"Exception during subtitle generation: {str(e)}")
...
async def get_summary(url: str, language: str = "en", length: str = "medium") -> str:
    ...
    try:
        ...
        logger.info(f"Generating summary for: {url}, language: {language}, length: {length}")
        if length.lower() not in ["short", "medium", "long"]:
            logger.error(f"Invalid summary length: {length}")
        ...
        logger.info(f"Checking if summary exists for: {video_id}, language: {language}, length: {length}")
        ...
        if not summary_status or summary_status.get("data", {}).get("status") != "completed":
            logger.info(f"Requesting summary generation for video ID: {video_id}")
            ...
            if not summary_request:
                # If we just generated a translation, try again with a delay
                if translation_needed:
                    logger.info("Translation was just completed, waiting before retrying summary request")
                    await asyncio.sleep(3)  # Small delay to let backend systems catch up
                ...
                # If still failing after retry
                if not summary_request:
                    logger.error(f"Failed to request {length} summary in {language}")
            ...
            while attempts < max_attempts:
                ...
                delay = await calculate_backoff_delay(attempts, base_delay=2.0, multiplier=1.5, max_delay=30.0)
                logger.info(f"Waiting {delay:.1f}s before checking summary status, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
                ...
                if not summary_status:
                    logger.info("Failed to check summary status, will retry")
                    continue
                ...
                if status == "processing":
                    logger.info(f"Summary generation still in progress")
        ...
        while attempts < max_attempts:
            ...
            if attempts > 0:
                delay = await calculate_backoff_delay(attempts - 1)
                logger.info(f"Waiting {delay:.1f}s before retrying summary retrieval, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
            
            # Get the summary
            logger.info(f"Retrieving summary for video ID: {video_id}")
            ...
            if summary_response and "status" in summary_response and summary_response["status"] == "success":
                ...
                if "data" in summary_response and "summary" in summary_response["data"]:
                    ...
                    logger.error("Unexpected response format: missing data.summary field")
...
async def search_video(url: str, query: str) -> str:
    ...
    """
    try:
        logger.info(f"Searching video: {url}, query: {query}")
        ...
        if not video_status or video_status.get("status") not in ["completed"]:
            ...
            logger.info(f"Video already processed, proceeding with search")
        ...
        while attempts < max_attempts:
            ...
            if attempts > 0:
                delay = await calculate_backoff_delay(attempts - 1)
                logger.info(f"Waiting {delay:.1f}s before retrying search, attempt {attempts+1}/{max_attempts}")
                await asyncio.sleep(delay)
            
            # Perform the search
            logger.info(f"Performing search for query: {query} in video ID: {video_id}")
            ...
            if search_response and "data" in search_response:
                ...
                if "results" not in search_data or not search_data["results"]:
                    logger.info(f"No matches found for '{query}' in the video")
                ...
                logger.info(f"Formatting search results with {matches} matches")
            ...
            if search_response and search_response.get("status") == "processing":
                logger.info(f"Search is still processing, will retry after backoff")
                attempts += 1
                continue
